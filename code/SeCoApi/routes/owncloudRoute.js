...
function OwnCloudRoute(configPath) {
    config = JSON.parse(fs.readFileSync(configPath));
    _initOwncloudClient();
    _initUserClient();
}

function _initOwncloudClient() {
    ...
    owncloudClient = new proto[config['grpc_service_name']](url,
...
OwnCloudRoute.prototype.route = function (router) {
    //Get File Route
    router.get('/owncloud/file', function (req, res) {
        _getFileRoute(req, res, _getRequestConfig('/owncloud/file', 'get'));
    });

    router.get('/owncloud/filetree', function (req, res) {
        _getFileTreeRoute(req, res, _getRequestConfig('/owncloud/filetree', 'get'));
    });

    router.put('/owncloud/file', function (req, res) {
        _uploadFileRoute(req, res, _getRequestConfig('/owncloud/file', 'put'));
    });

    router.post('/owncloud/file/transfer', function (req, res) {
        _fileTransfer(req, res, _getRequestConfig('/owncloud/file/transfer', 'post'));
    });
...
function _fileTransfer(req, res, requestConfig) {
    ...
    _handleAuth(config, req.username, function (err, token) {
        ...
        owncloudClient[requestConfig['grpc_function']](jsonGrpcArgs, function (err, response) {
            if (err) {
...
function _uploadFileRoute(req, res, requestConfig) {
    ...
    _handleAuth(config, req.username, function (err, token) {
        ...
        var call = owncloudClient[requestConfig['grpc_function']](metadata,function (err, response) {
            if (err) {
                return _grpcError(res, config['service_name'], err);
            } else {
                if (response.err) {
...
function _getFileRoute(req, res, requestConfig) {
    ...
    _handleAuth(config, req.username, function (err, token) {
        ...
        winston.log('info', 'authToken: ', authToken);
        var jsonGrpcArgs = _createGrpcJsonArgs(config, req, requestConfig['query_parameter'], requestConfig['reserved_parameter'], authToken);
        var call = owncloudClient.getFile(jsonGrpcArgs);
