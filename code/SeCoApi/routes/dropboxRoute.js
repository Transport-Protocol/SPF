...
function DropboxRoute(configPath) {
    config = JSON.parse(fs.readFileSync(configPath));
    _initDropboxClient();
    _initUserClient();
}

function _initDropboxClient() {
    ...
    dropboxClient = new proto[config['grpc_service_name']](url,
...
DropboxRoute.prototype.route = function (router) {
    //Get File Route
    router.get('/dropbox/file', function (req, res) {
        _getFileRoute(req, res, _getRequestConfig('/dropbox/file', 'get'));
    });

    router.get('/dropbox/filetree', function (req, res) {
        _getFileTreeRoute(req, res, _getRequestConfig('/dropbox/filetree', 'get'));
    });

    router.put('/dropbox/file', function (req, res) {
        _uploadFileRoute(req, res, _getRequestConfig('/dropbox/file', 'put'));
    });
...
function _fileTransfer(req, res, requestConfig) {
    ...
    _handleAuth(config, req.username, function (err, token) {
        ...
        dropboxClient[requestConfig['grpc_function']](jsonGrpcArgs, function (err, response) {
            if (err) {
...
function _uploadFileRoute(req, res, requestConfig) {
    ...
    _handleAuth(config, req.username, function (err, token) {
        ...
        var call = dropboxClient[requestConfig['grpc_function']](metadata,function (err, response) {
            if (err) {
                return _grpcError(res, config['service_name'], err);
            } else {
                if (response.err) {
...
function _getFileRoute(req, res, requestConfig) {
    ...
    _handleAuth(config, req.username, function (err, token) {
        ...
        winston.log('info', 'authToken: ', authToken);
        var jsonGrpcArgs = _createGrpcJsonArgs(config, req, requestConfig['query_parameter'], requestConfig['reserved_parameter'], authToken);
        var call = dropboxClient.getFile(jsonGrpcArgs);
